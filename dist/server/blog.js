...
router.get('/getByClass', function (req, res, next) {
    ...
    if (sqlClass) {
        var searchBlogByClass = 'select * from blog b ' + 'right join paper_tag p on b.id = p.blog_id ' + 'left join ( select blog_id as cBlog_id , count(id) as comment_count from comment group by blog_id) c on c.cBlog_id = b.id' + ' where class = "' + sqlClass + '"';
        ...
        connection.query(searchBlogByClass, function (err, result) {
            if (err) {
                ...
                if (result.length == 0) {
                    ...
                    // console.log(blogId);
                    var searchId = function searchId(data, id) {
                    ...
                    for (var i = 0; i < result.length; i++) {
                        if (blogId.indexOf(result[i].blog_id) >= 0) {} else {
...
router.post('/newComment', function (req, res, next) {
    ...
    if (content && email && blog_id && nickname) {
        //insert query
        var insertComment = 'insert into comment (blog_id,email,nickname,content) value(' + '\"' + blog_id + '\",' + '\"' + email + '\",' + '\"' + nickname + '\",' + '\"' + content + '\"' + ')';
        connection.query(insertComment, function (err, result) {
            if (err) {
                ...
                console.log(result);
                var searchComment = 'select * from comment where blog_id=\"' + blog_id + '\"';
